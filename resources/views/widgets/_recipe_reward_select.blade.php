    ...
    <tbody id="rewardTableBody">
        @if($rewards)
            @foreach($rewards as $reward)
                <tr class="reward-row">
                    <td>{!! Form::select('rewardable_type[]', ['Item' => 'Item', 'Currency' => 'Currency', 'LootTable' => 'Loot Table', 'Raffle' => 'Raffle','Award' => 'Award',], $reward->rewardable_type, ['class' => 'form-control reward-type selectize', 'placeholder' => 'Select Reward Type']) !!}</td>
                    <td class="reward-row-select">
                        ...
                        @elseif($reward->rewardable_type == 'Award')
                            {!! Form::select('rewardable_id[]', $awards, $reward->rewardable_id, ['class' => 'form-control award-select selectize', 'placeholder' => 'Select Award']) !!}
                        @endif
                    </td>
